...
class CancleOrderScreen extends StatefulWidget {
  const CancleOrderScreen({super.key});

  @override
  State<CancleOrderScreen> createState() => _CanceledOrdersScreenState();
}

class _CanceledOrdersScreenState extends State<CancleOrderScreen> {
    ...
    return Scaffold(
      ...
      body: StreamBuilder<List<OrderModel>>(
        ...
        builder: (context, snapshot) {
          ...
          final canceledOrders = (snapshot.data ?? [])
  ...
  Widget _buildExpandableOrderCard(OrderModel order) {
    return ExpansionTile(
      ...
      subtitle: Text(
        "Canceled on ${order.orderDate != null ? dateFormat.format(order.orderDate!) : '-'}",
        style: const TextStyle(fontSize: 12, color: blackColor60),
      ),
      childrenPadding: const EdgeInsets.all(16),
      children: [
        const OrderProgress(
          orderStatus: OrderProcessStatus.canceled,
          processingStatus: OrderProcessStatus.canceled,
          packedStatus: OrderProcessStatus.canceled,
          shippedStatus: OrderProcessStatus.canceled,
          deliveredStatus: OrderProcessStatus.canceled,
          isCanceled: true,
        ),
